...
// Формировать гору на клетке, где уже есть гора можно. Будет самая обычная гора
...
// Пусть размеры поля (SIZE x y). Тогда при установке позиции динозавра (START x1 y1) значения должны принимать такие значения: 0 <= x1 <= x - 1, 0 <= y1 <= y.

// Данная лабороторная работа выполнено вся до пункта 6.3 (не включительно). Проверялась она на macOS.
